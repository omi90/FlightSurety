...
let oracleAccountsList = [];
let oraclesIndexesList = [];

const ORACLES_COUNT_FORTEST = 25;
...
flightSuretyApp.events.OracleRequest({
    ...
    let idx = 0;
    oraclesIndexesList.forEach((indexes) => {
      let oracle = oracleAccountsList[idx];
...
function getOracleAccounts() {
  return new Promise((resolve, reject) => {
    web3.eth.getAccounts().then(accountList => {
      ...
      oracleAccountsList = accountList.slice(20, 20+ORACLES_COUNT_FORTEST);
    }).catch(err => {
      reject(err);
    }).then(() => {
...
function initOracles(accounts) {
  return new Promise((resolve, reject) => {
    flightSuretyApp.methods.REGISTRATION_FEE().call().then(fee => {
      for(let a=0; a<ORACLES_COUNT_FORTEST; a++) {
        flightSuretyApp.methods.registerOracle().send({
          ...
          flightSuretyApp.methods.getMyIndexes().call({
            ...
            oraclesIndexesList.push(result);
          }).catch(err => {
            reject(err);
          });
        }).catch(err => {
